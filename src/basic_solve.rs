...
pub fn solve_mcq_back(
    ...
    if sheets_raw.len() != grades.len() {
        return Err(format!(
            "Error: The number of answer sheets ({}) is not the number of grades ({}) given", sheets_raw.len(), grades.len()
        ));
    }
    ...
    for k in 0..simulation_sheets {
        ...
        if sheet.len() != number_of_questions {
            return Err(format!("Error: The answer sheet at index {} has {} answers but the first one provided had {}",
                k, sheet.len(), number_of_questions
            ));
        }
