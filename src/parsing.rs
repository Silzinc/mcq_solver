...
{
	fn parse_into_grades(&self) -> Result<Vec<u8>, String>
	{
		...
		{
			Err(format!(
				"Grades parsing: Not enough grades were found in the input string (expected {}, found {})",
	...
	fn parse_into_answers(&self, index: usize) -> Result<Vec<Answer>, String>
	{
		let input = match fs::read_to_string(&self.path_to_sheets[index])
		{
			...
			{
				return Err(format!(
					"Parsing answers: failed to load {}",
					self.path_to_sheets[index]
				))
			}
		};
		...
		{
			Err(format!(
				"Parsing answers: Not enough answers were found in the input string (expected {}, found \
				 {})",
				params.number_of_questions,
				answers.len()
			))
